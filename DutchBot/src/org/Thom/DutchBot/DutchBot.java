...
public class DutchBot extends PircBot {
	...
	public DutchBot(String nickservPassword, String name) {
		...
		this.setName(name);
		setVersion(name + " " + VERSION + " by DutchDude");
		setLogin("name");
		
		this.getTimer().schedule(new ConnectionProtectorTask(this), 1000L, 1000L);
	}
	...
	public final boolean tryConnect(String server, int port, final String nick,
		...
		if (!this.isConnected()) {
			try {
				...
				GhostTask gt = new GhostTask(this.getTimer(), this, nick);
	...
	public void addAutoJoinList(String channel) {
